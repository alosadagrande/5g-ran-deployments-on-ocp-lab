...
In the previous sections we have deployed the `sno2` cluster using an link:crafting-deployments-iac.html#crafting-our-own-clusterinstance[agent-based installation] and the `sno3` cluster following an link:installing-imagebaseinstall-cluster.html[image-based methodology]. Before we start the upgrade we need to check if the Lifecycle Agent and OpenShift API for Data Protection operators are installed in the target clusters.
