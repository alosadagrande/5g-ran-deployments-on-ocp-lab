...
IMPORTANT: If you check the binding rules, you see that we are targeting clusters labeled with `common: "{policygen-common-label}"` and `logicalGroup: "active"`. These labels were set in the `ClusterInstance` definition in the link:crafting-deployments-iaac.html#siteconfig[previous section].
